 ...
 */
function updateMenuTriggerBgColorLabel(controlId: string, buttonStyle: string, device: string) {
 ...
 */
function applyMenuTriggerColorToPreview(device: "desktop" | "mobile", buttonStyle: string, colorValue: any) {
	try {
		const selector = device === 'mobile' ? '.wpbf-mobile-menu-toggle' : '.wpbf-menu-toggle';
		...
		if (buttonStyle === 'outline') {
			...
			$els.css({ 'background-color': 'unset', 'border': '2px solid ' + colorValue });
