...
/**
 * Lazy registration handler entry.
 */
type LazyHandler = {
	setup: () => void;
	initialized: boolean;
...
/**
 * Registry mapping section IDs to handler setup functions.
 * Handlers are only registered when their section is first expanded.
 */
const sectionHandlers: Record<string, LazyHandler> = {};

/**
 * Track which panels have been registered for lazy loading.
 * Panel expansion triggers all child section handlers.
 */
const panelHandlers: Record<string, LazyHandler[]> = {};

/**
 * Register a handler function for lazy initialization when a section expands.
 *
 * @param sectionId - The section ID to register the handler for.
 * @param setup - The setup function to call when the section is expanded.
 */
export function registerSectionHandler(sectionId: string, setup: () => void) {
	sectionHandlers[sectionId] = { setup, initialized: false };
}

/**
 * Register a handler to run when any section in a panel first expands.
 *
 * @param panelId - The panel ID to register the handler for.
 * @param setup - The setup function to call when a section in the panel is expanded.
 */
export function registerPanelHandler(panelId: string, setup: () => void) {
	if (!panelHandlers[panelId]) {
		panelHandlers[panelId] = [];
	}
	panelHandlers[panelId].push({ setup, initialized: false });
}

/**
 * Initialize a section's handler if not already initialized.
 *
 * @param sectionId - The section ID to initialize.
 */
export function initializeSectionHandler(sectionId: string) {
...
/**
 * Initialize all handlers registered for a panel (run on first child section expand).
 *
 * @param panelId - The panel ID to initialize handlers for.
 */
export function initializePanelHandlers(panelId: string) {
	const handlers = panelHandlers[panelId];
	if (handlers) {
