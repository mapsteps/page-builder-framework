...
})({"gbEIx":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "1b6c17c12ef6bea6";
"use strict";
...
},{}],"1kavp":[function(require,module,exports) {
...
},{"828076ff57d2bad7":"eiXKv","5ce303497de57ed6":"igwuz"}],"eiXKv":[function(require,module,exports) {
"use strict";
module.exports = require("3f1916c5c33da7b0");

},{"3f1916c5c33da7b0":"3DtMI"}],"3DtMI":[function(require,module,exports) {
...
},{}],"igwuz":[function(require,module,exports) {
...
},{"3dd49ed909d5c9c3":"dFaqQ"}],"dFaqQ":[function(require,module,exports) {
...
},{}],"7Jyqk":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
var _builderControlScss = require("./builder-control.scss");
var _headerBuilderControl = require("./header-builder-control");
var _headerBuilderControlDefault = parcelHelpers.interopDefault(_headerBuilderControl);
wp.customize.controlConstructor["wpbf-header-builder"] = (0, _headerBuilderControlDefault.default);

},{"./builder-control.scss":"fsShp","./header-builder-control":"bW7KL","@parcel/transformer-js/src/esmodule-helpers.js":"avPBH"}],"fsShp":[function() {},{}],"bW7KL":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _stringUtil = require("../../Generic/src/string-util");
const HeaderBuilderControl = wp.customize.controlConstructor["wpbf-header-builder"] = wp.customize.wpbfDynamicControl.extend({
    form: undefined,
    valueField: undefined,
    draggableData: undefined,
    emptyWidgetItemMarkup: "<div class='widget-item empty-widget-item'>&nbsp;</div>",
    availableWidgetsPanel: undefined,
    builderPanel: undefined,
    initWpbfControl: function(control) {
        control = control || this;
        if (!control) return;
        const controlForm = document.querySelector(`#_customize-input-${control.id} .wpbf-control-form`);
        if (controlForm instanceof HTMLElement) control.form = controlForm;
        /**
     ...
     */ control.valueField?.addEventListener("change", function() {
            ...
            const parsedJson = (0, _stringUtil.parseJsonOrUndefined)(this.value);
    ...
    isSortableEmpty: function(el) {
        ...
        for(let i = 0; i < children.length; i++){
            ...
            if (child.classList.contains("empty-widget-item")) continue;
            if (child.classList.contains("ui-sortable-placeholder")) continue;
            if (child.classList.contains("ui-sortable-helper")) continue;
            return false;
        }
        return true;
    },
    isWidgetActive: function(widgetKey) {
        ...
        for(const rowKey in value){
            ...
            for(const columnKey in row){
                ...
                if (column.includes(widgetKey)) return true;
            }
        }
        return false;
    },
    ...
    buildAvailableWidgetsPanel: function() {
        ...
        if (!params) return;
        const availableWidgetsPanel = control.container[0].querySelector(`.available-widgets-panel`);
        if (!(availableWidgetsPanel instanceof HTMLElement)) return;
        ...
        const $availableWidgetListEl = jQuery("<div></div>").addClass("header-builder-widgets available-widgets").appendTo(control.availableWidgetsPanel);
        // Build the available widgets list based on `availableWidgets`.
        availableWidgets.forEach((widget)=>{
            const widgetKey = widget.key;
            jQuery("<div></div>").addClass(`widget-item widget-item-${widgetKey} ${control.isWidgetActive?.(widgetKey) ? "disabled" : ""}`).attr("data-widget-key", widgetKey).attr("draggable", "true").html(`<span class="widget-label">${widget.label}</span>`).appendTo($availableWidgetListEl);
        });
    },
    buildBuilderPanel: function() {
        ...
        if (!(customizePreview instanceof HTMLElement)) return;
        // Create the panel.
        const $headerBuilderPanel = jQuery("<div></div>").addClass("header-builder-panel").insertAfter(customizePreview);
        availableRows.forEach((row)=>{
            // Build the row.
            const $row = jQuery("<div></div>").addClass(`builder-row`).attr("data-row-key", row.key).appendTo($headerBuilderPanel);
            // Build the inner row.
            const $innerRow = jQuery("<div></div>").addClass(`builder-inner-row`).appendTo($row);
            // Build the row setting button.
            jQuery("<button></button>").addClass("row-setting-button").attr("data-row-key", row.key).html('<i class="dashicons dashicons-admin-generic"></i>').appendTo($row);
            const matchedRow = params.value[row.key];
            row.columns.forEach((column, columnIndex)=>{
                const columnPosClass = columnIndex === 0 ? "column-start" : columnIndex === row.columns.length - 1 ? "column-end" : "column-middle";
                const $widgetListEl = jQuery("<div></div>").addClass(`header-builder-widgets builder-column sortable-widgets ${columnPosClass}`).attr("data-column-key", column.key).appendTo($innerRow);
                ...
                matchedColumn.forEach((widgetKey)=>{
                    ...
                    jQuery("<div></div>").addClass(`widget-item widget-item-${widgetKey}`).attr("data-widget-key", widgetKey).html(`<span class="widget-label">${matchedWidget.label}</span>`).appendTo($widgetListEl);
                });
            });
        });
    ...
    initDraggable: function() {
        ...
        const availableWidgetItems = control.availableWidgetsPanel?.querySelectorAll(".widget-item");
        if (!availableWidgetItems || !availableWidgetItems.length) return;
    ...
    initDroppable: function() {
        ...
        if (!params) return;
        if (!control.availableWidgetsPanel || !control.builderPanel) return;
        const builderDropZones = control.builderPanel?.querySelectorAll(".builder-column");
        if (!builderDropZones || !builderDropZones.length) return;
        builderDropZones.forEach((dropZone)=>{
            ...
            dropZone.addEventListener("drop", function(e) {
                ...
                const temporaryWidgetItem = dropZone.querySelector(".ui-sortable-placeholder.from-available-widgets");
                if (temporaryWidgetItem) dropZone.removeChild(temporaryWidgetItem);
                if (dropZone.classList.contains("column-end")) dropZone.insertBefore(newWidgetItem, dropZone.firstChild);
                else dropZone.appendChild(newWidgetItem);
                const emptyWidgetItem = dropZone.querySelector(".empty-widget-item");
                if (emptyWidgetItem) emptyWidgetItem.remove();
    ...
    getWidgetItemFromDraggableData: function(e) {
        ...
        const widgetItem = this.availableWidgetsPanel?.querySelector(`.widget-item[data-widget-key="${widgetKey}"]`);
    ...
    initSortable: function() {
        ...
        jQuery(".sortable-widgets").sortable({
            ...
            start: function(e, ui) {
                ...
                if (labelEl instanceof HTMLElement) ui.placeholder[0].appendChild(labelEl.cloneNode(true));
            },
    ...
    handleSortableSortout: function(sortableEl) {
        ...
        const control = this;
        if (!control.availableWidgetsPanel || !control.builderPanel) return;
        const emptyWidgetItem = sortableEl.querySelector("." + emptyWidgetItemClass);
        if (control.isSortableEmpty?.(sortableEl)) {
            sortableEl.classList.add(emptyWidgetListClass);
            if (!emptyWidgetItem) jQuery(sortableEl).append(control.emptyWidgetItemMarkup ?? "");
        } else {
    ...
    updateComponentState: function(value) {
        ...
        if (!textarea) return;
        textarea.value = (0, _stringUtil.encodeJsonOrDefault)(value);
    },
    valuesEqual: function(a, b) {
        const aJson = (0, _stringUtil.encodeJsonOrDefault)(a);
        const bJson = (0, _stringUtil.encodeJsonOrDefault)(b);
        return aJson === bJson;
    }
});
exports.default = HeaderBuilderControl;

},{"../../Generic/src/string-util":"5ljKF","@parcel/transformer-js/src/esmodule-helpers.js":"avPBH"}],"5ljKF":[function(require,module,exports) {
/**
 ...
 */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "encodeJsonOrDefault", ()=>encodeJsonOrDefault);
/**
 ...
 */ parcelHelpers.export(exports, "parseJsonOrUndefined", ()=>parseJsonOrUndefined);
function encodeJsonOrDefault(value) {
    try {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"avPBH"}],"avPBH":[function(require,module,exports) {
...
},{}]},["gbEIx","1kavp","7Jyqk"], "7Jyqk", "parcelRequire9ea3")
