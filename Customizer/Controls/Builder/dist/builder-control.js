...
const HeaderBuilderControl = wp.customize.controlConstructor["wpbf-header-builder"] = wp.customize.wpbfDynamicControl.extend({
    ...
    initWpbfControl: function(control) {
        ...
        if ("wpbf-header-builder" !== params.type) return;
    ...
    buildAvailableWidgetsPanel: function() {
        ...
        const $availableWidgetsEl = jQuery("<div></div>").addClass("header-builder-widgets available-widgets").appendTo(control.availableWidgetsPanel);
        ...
        for(const widgetKey in availableWidgets){
            ...
            jQuery("<div></div>").addClass(`widget-item widget-item-${widgetKey} ${activeWidgets[widgetKey] ? "disabled" : ""}`).attr("data-widget-key", widgetKey).attr("draggable", "true").html(`<span class="widget-label">${availableWidgets[widgetKey]}</span>`).appendTo($availableWidgetsEl);
        }
    },
    buildBuilderPanel: function() {
        ...
        for(const rowKey in availableRows){
            ...
            const $row = jQuery("<div></div>").addClass(`builder-row builder-${rowKey}`).attr("data-row-key", rowKey).appendTo($headerBuilderPanel);
            const $rowWidgetsEl = jQuery("<div></div>").addClass(`header-builder-widgets builder-widgets builder-widgets-${rowKey} sortable-widgets`).appendTo($row);
            const matchedRow = params.value[rowKey];
            if (!matchedRow || !Object.keys(matchedRow).length) {
                $rowWidgetsEl.html("<div class='widget-item empty-widget-item'>&nbsp;</div>");
                continue;
            }
            ...
            for(const widgetKey in matchedRow){
                ...
                jQuery("<div></div>").addClass(`widget-item widget-item-${widgetKey}`).attr("data-widget-key", widgetKey).html(`<span class="widget-label">${availableWidgets[widgetKey]}</span>`).appendTo($rowWidgetsEl);
            }
        }
    ...
    initSortable: function() {
        ...
        if (availableWidgetItems && availableWidgetItems.length) availableWidgetItems.forEach((item)=>{
        ...
        jQuery(".builder-widgets").sortable({
            ...
            update: function(event, ui) {
                ...
                if (!totalChildren) jQuery(sortableEl).append("<div class='widget-item empty-widget-item'>&nbsp;</div>");
            },
            stop: function(event, ui) {
                document.body.classList.remove("is-sorting-widget");
            }
        });
        const builderDropZones = control.builderPanel?.querySelectorAll(".builder-widgets");
        if (builderDropZones && builderDropZones.length) builderDropZones.forEach((dropZone)=>{
...
//# sourceMappingURL=builder-control.js.map
