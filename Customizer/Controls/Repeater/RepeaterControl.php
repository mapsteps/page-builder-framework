...
class RepeaterControl extends BaseControl {
	...
	public function __construct( $manager, $id, $args = [] ) {
		...
		$this->row_label = [
			'type'  => 'text',
			'value' => esc_attr__( 'row', 'page-builder-framework' ),
			'field' => false,
		];
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							[
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_html__( 'Select a Page', 'page-builder-framework' ),
								'option_none_value' => '0',
								'selected'          => '',
							]
						);
	...
	public function to_json() {
		...
		$this->json['rowLabel']    = $this->row_label;
		...
		$this->json['value'] = apply_filters( "wpbf_controls_repeater_value_{$this->id}", $this->json['value'] );
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<div class="actions">
									...
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'page-builder-framework' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'page-builder-framework' ); ?>" >
										...
										<# } else { #>
											<?php esc_html_e( 'Add Image', 'page-builder-framework' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<div class="actions">
									...
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'page-builder-framework' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'page-builder-framework' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change File', 'page-builder-framework' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add File', 'page-builder-framework' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
	 ...
	 */
	protected function validate_row_label( $args ) {
